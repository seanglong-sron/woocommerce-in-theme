...
function porto_grid_list_toggle() {
    ...
    if ( !isset( $porto_woocommerce_loop['view'] ) ) {
?>
...
function porto_loop_product_thumbnail() {
    ...
    $feature2_ids = get_post_meta($id,'dfiFeatured',true);
    $feature2 = explode(',', $feature2_ids[0]);
    ...
    if (!empty($gallery) && $porto_settings['category-image-hover']) {
        ...
        if ( $show_hover_img ) {
            $first_image_id = $thumbnail_id + 1 ; // $gallery[0];
            if($feature2[1] == "" || empty($feature2[1])){
                //$attachment_image = get_the_post_thumbnail($id , $size, array('class' => ''));
            }else{
            	$attachment_image = "<img src='".site_url('wp-content/uploads/'.$feature2[1])."' class='hover-image' />";
            }
            //wp_get_attachment_image($first_image_id , $size, false, array('class' => 'hover-image'));
        }
    }
